...
//
// P3D Course
// (c) 2026 by Jo�o Madeiras Pereira
		...
		for (int y = 0; y < RES_Y; y++) {
			for (int x = 0; x < RES_X; x++) {
				...
				if (drawModeEnabled) {
					...
					index_col = 3 * (x + RES_X * y);
					img_Data[index_col] = u8fromfloat((float)color.r());
					img_Data[index_col + 1] = u8fromfloat((float)color.g());
					img_Data[index_col + 2] = u8fromfloat((float)color.b());
				}
			}
		}
...
int main(int argc, char* argv[])
{
	...
	if (!drawModeEnabled) {
		if (Progressive_flg) {
			printf("Progressive Mode enabled:not allowed to write image files.Turn it off.\n\n");
			exit(0);
		}
		int ch;
		printf("Rendering to an image file.........\n\n");
		do {
			init_scene();
